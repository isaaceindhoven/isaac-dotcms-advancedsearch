...
import nl.isaac.dotcms.searcher.util.StatusValidator;
...
public class SearcherFilter {
	...
	public SearcherFilter(UserSearchValues userSearchValues) {
		...
		this.textUtil = new TextUtil(userSearchValues.getSearchMode().toString(), userSearchValues.getSnippetSizeBefore(), userSearchValues.getSnippetSizeAfter(), userSearchValues.getExcludeText());
	...
	public boolean spotsLeft() {
		if (userSearchValues.searchAllResults()) return true;
		return userSearchValues.getMaxResults() > filteredPortletHitsSize;
	}
	
	public Integer spotsLeftAmount() {
		if (userSearchValues.getMaxResults() == null) return null;
		return userSearchValues.getMaxResults() - filteredPortletHitsSize;
	}
	...
	// Only content or widget, no file
	private Collection<PortletHit> filterContentlet(Type type, Contentlet contentlet) {
		...
		if (status.isValid()) {
			...
			return filterAttributesAndGetHits(SearchableAttributesUtil.getContentletAttributes(type, contentlet), host.getHostname(), row, status.getActualStatus());
		}
	...
	private Collection<PortletHit> filterContainer(Container container) {
		...
		if (status.isValid()) {
			return filterAttributesAndGetHits(SearchableAttributesUtil.getContainerAttributes(container), host.getHostname(), status.getObject(), status.getActualStatus());
		}
	...
	private Collection<PortletHit> filterTemplate(Template template) {
		...
		if (status.isValid()) {
			return filterAttributesAndGetHits(SearchableAttributesUtil.getTemplateAttributes(template), host.getHostname(), status.getObject(), status.getActualStatus());
		}
	...
	private Collection<PortletHit> filterStructure(Structure structure) {
		return filterAttributesAndGetHits(SearchableAttributesUtil.getStructureAttributes(structure), host.getHostname(), structure, this.userSearchValues.getStatus());
	}
	...
	private Collection<PortletHit> filterHtmlPage(HTMLPage htmlPage) {
		...
		if (status.isValid()) {
			...
			return filterAttributesAndGetHits(SearchableAttributesUtil.getHtmlPageAttributes(htmlPage), host.getHostname(), row, status.getActualStatus());
		}
	...
	private Collection<PortletHit> filterFolder(Folder folder) {
		...
		return filterAttributesAndGetHits(SearchableAttributesUtil.getFolderAttributes(folder), host.getHostname(), folder, status);
	}
	
	private Collection<PortletHit> filterAttributesAndGetHits(Collection<SearchableAttribute> searchableAttributes, String hostName, Object object, Status status) {
		...
		for (SearchableAttribute attribute : searchableAttributes) {
			...
			if (textUtil.checkValueForText(attribute.getValue(), this.userSearchValues.getSearchString())) {
				portletHits.add(new PortletHit(attribute.getType(), hostName, object, status, attribute.getTitle(), attribute.getName(), attribute.getValue()));
